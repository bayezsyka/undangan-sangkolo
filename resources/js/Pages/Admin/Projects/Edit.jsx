...
export default function Edit({ auth, project, clients, active_slots_count }) {
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title={`Edit Project: ${project.name_project}`} />
            
            <PageHeader 
                title="Edit Project" 
                subtitle={`Memperbarui pengerjaan untuk ${project.name_project}`}
                actions={
            ...
            <div className="max-w-4xl">
                <form onSubmit={submit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <SectionCard title="Informasi Project" description="Sesuaikan rencana acara atau detail klien.">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-1">
                                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest pl-1">Client</label>
                                    <select 
                        ...
                        <SectionCard title="Status & Slot">
                            <div className="space-y-6">
                                ...
                                <div className="pt-6 border-t border-gray-50">
                                    <label className={`flex items-center gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-100 transition-colors group ${ (active_slots_count < 3 || data.is_active_slot) ? 'cursor-pointer hover:bg-white' : 'opacity-50 grayscale'}`}>
                                        <div className="flex-1">
                                            ...
                                            <p className="text-[10px] text-gray-400 leading-tight font-bold italic">
                                                { (active_slots_count < 3 || data.is_active_slot) ? 'Aktifkan slot prioritas untuk project ini.' : 'Slot prioritas penuh. Tunggu slot kosong.'}
                                            </p>
                                        </div>
                                        <div className="relative">
                                            <input 
                                                ...
                                                disabled={active_slots_count >= 3 && !data.is_active_slot}
                                ...
                                >
                                    {processing ? 'Proses...' : 'Perbarui Project'}
                                </button>
                            </div>
                        </SectionCard>
                    </div>
